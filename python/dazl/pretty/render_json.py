...
from ..damlast.daml_lf_1 import DefDataType, DefTemplate, Expr, Module, Package, PrimType, Type
from ..model.types import ModuleRef, TypeReference
from ..pretty.util import indent
...
class JsonPrettyPrint(AstVisitor[JsonValue, JsonValue, JsonValue, JsonValue]):
    ...
    def render_store(self) -> str:
        ...
        required_types = [
            repr(t)
            for t in find_dependencies(
                self.store,
                [TypeReference.parse(template_id) for template_id in base_dict["templates"]],
            )
        ]
        print(required_types)

        defs = {
            key: value for key, value in base_dict["definitions"].items() if key in required_types
        }
    ...
    def visit_module(self, module: "Module") -> "JsonValue":
        def type_name(segments) -> str:
            return (
                ".".join(self.context.current_module.module_name)
                + ":"
                + ".".join(segments)
                + "@"
                + self.context.current_module.package_id
            )

        return dict(
            templates={
                type_name(template.tycon.segments): self.visit_def_template(template)
                for template in module.templates
            },
            definitions={
                type_name(data_type.name.segments): self.visit_def_data_type(data_type)
    ...
    def visit_def_data_type(self, data_type: "DefDataType") -> "JsonValue":
        if data_type.record:
            return {
                "record": {fwt.field: self.visit_type(fwt.type) for fwt in data_type.record.fields}
            }
        elif data_type.variant:
            return {
                "variant": {
                    fwt.field: self.visit_type(fwt.type) for fwt in data_type.variant.fields
                }
            }
        elif data_type.enum:
