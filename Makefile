...
proto_rel_dir  := $(shell $(protos) list --kind dir)
proto_rel_pb   := $(shell $(protos) list --kind pb)
proto_rel_grpc := $(shell $(protos) list --kind grpc)
proto_src_pb   := $(foreach p,$(proto_rel_pb),.cache/protos/$(p))
proto_src_grpc := $(foreach p,$(proto_rel_pb),.cache/protos/$(p))

# proto: the (slightly modified) Protobuf files from Daml Connect
$(proto_src_pb) $(proto_src_grpc): .cache/witnesses/proto
.cache/witnesses/proto:
	$(protos) unpack
	@mkdir -p $(@D) && touch $@
...
py_src_gen_grpc := $(foreach d,$(proto_rel_grpc),$(py_src_gen_root)/$(d:.proto=_pb2.py)) \
                   $(foreach d,$(proto_rel_grpc),$(py_src_gen_root)/$(d:.proto=_pb2_grpc.py))
...
# python: Protobuf generated code (non-gRPC)
$(py_src_gen_pb): $(py_src_gen_root)/%: $(py_tmp_gen_root)/% COPYRIGHT .cache/witnesses/python-pb
	@mkdir -p $(@D)
	cp $< $@
...
$(py_src_gen_grpc): $(py_src_gen_root)/%: $(py_tmp_gen_root)/% COPYRIGHT .cache/witnesses/python-grpc
	@mkdir -p $(@D)
	cp $< $@
...
gen-go: .cache/bin/protoc-gen-go .cache/bin/protoc-gen-go-grpc
